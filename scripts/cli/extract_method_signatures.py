...
def parse_google_docstring(docstring: Optional[str]) -> Dict[str, Dict[str, str]]:
    ...
    for match in re.finditer(param_pattern, args_section):
        ...
        params[name] = {"type": type_info.strip(), "description": description.strip(), "optional": is_optional}
...
def get_parameter_info(func: Any) -> List[Dict[str, Any]]:
    ...
    for name, param in signature.parameters.items():
        ...
        if name in docstring_info:
            param_info.update({"type": docstring_info[name]["type"], "description": docstring_info[name]["description"]})
            # Override required based on docstring if marked as optional
            if docstring_info[name].get("optional"):
                param_info["required"] = False
        else:
            param_info.update({"type": str(param.annotation) if param.annotation != inspect.Parameter.empty else "Unknown", "description": ""})
